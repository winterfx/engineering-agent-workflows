...
describe("generated Scheduler artifact build", () => {
  ...
  it("embeds the deterministic tool policy in a scheduler artifact", () => {
    const artifact = renderSchedulerArtifact(
      ['const WORKFLOW_POLICY_JSON = "__WORKFLOW_POLICY_JSON__";', ""].join(
        "\n",
      ),
      '{"enabled":true}\n',
    );
    ...
    expect(() => renderSchedulerArtifact("const value = 1;", "{}")).toThrow(
      "exactly one __WORKFLOW_POLICY_JSON__ placeholder",
    );
  });
